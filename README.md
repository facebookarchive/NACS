...
usage: __main__.py [-h] --mode {train,test} [--model_type {encdec,model1}]
                   ...
                   [--n_symbols N_SYMBOLS] [--reward_type {prob,logprob,01}]
                   [--predict_word_separately PREDICT_WORD_SEPARATELY]
                   [--context_start_iter CONTEXT_START_ITER] [--ctx CTX]
                   [--ctx_dropout CTX_DROPOUT] [--ctx_detach CTX_DETACH]
                   [--eval_random_sym EVAL_RANDOM_SYM]
                   [--eval_argmin_sym EVAL_ARGMIN_SYM] [--coeff_ce COEFF_CE]
                   [--coeff_rl COEFF_RL]
                   [--coeff_rl_baseline COEFF_RL_BASELINE]
                   ...
                   [--symbol_word_gate SYMBOL_WORD_GATE]
                   [--gumbel_hard GUMBEL_HARD] [--gumbel_tau GUMBEL_TAU]
                   [--gumbel_tau_decay GUMBEL_TAU_DECAY]
                   [--gumbel_tau_decay_steps GUMBEL_TAU_DECAY_STEPS]
                   [--mtl MTL] [--src_tags SRC_TAGS] [--trg_tags TRG_TAGS]
                   [--emb_dim_tags EMB_DIM_TAGS] [--ctx_gate CTX_GATE]
                   [--ctx_dim CTX_DIM]
```
